...
const APP_SCHEME = process.env.APP_SCHEME || "podscan";
const PUBLIC_BASE_URL = (process.env.PUBLIC_BASE_URL || `http://localhost:${PORT}`).replace(/\/$/, "");
const FROM_EMAIL = process.env.FROM_EMAIL || "PodScan <onboarding@resend.dev>";
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1a0f0a;line-height:1.5">
          ...
          <p><a href="${magicUrl}" style="color:#c8860a;font-weight:700">Open PodScan</a></p>
...
const server = createServer(async (req, res) => {
  try {
    ...
    html(res, 404, "<h1>PodScan Auth</h1><p>Not found.</p>");
  } catch (error) {
    console.error(error);
...
server.listen(PORT, () => {
  console.log(`PodScan auth listening on ${PORT}`);
});
